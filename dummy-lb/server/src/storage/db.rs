...
impl Database for PGDatabase {
    ...
    fn new_user_finalize(
        ...
        let dbr = self.database_r()?;
        let statement =
            dbr.prepare(&format!("SELECT * FROM {} WHERE statechainid = $1", Table::UserSession.to_string(),))?;
        let rows = statement.query(&[&statechain_id])?;

        let row = rows.get(0);

        let s2: String = row.get("s2");
        let tx: String = row.get("txbackup");
        let pk: String = row.get("proofkey");
        let auth: String = row.get("authentication");
