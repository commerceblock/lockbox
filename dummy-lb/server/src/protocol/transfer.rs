...
pub trait Transfer {
    ...
    fn keyupdate_first(&self, ku_send: KUSendMsg) -> Result<KUReceiveMsg>;
    ...
    fn keyupdate_second(&self, ku_fin: KUFinalize) -> Result<KUAttest>;
...
//        else {
            ...
            let p1_pub = kp.party_2_public;
            let p2_pub = transfer_msg4.o2_pub;
...
//        }
    ...
    fn keyupdate_first(&self, ku_send: KUSendMsg) -> Result<KUReceiveMsg> {
        let user_id = ku_send.user_id;
        let statechain_id = ku_send.statechain_id;

        println!("User ID: {:?}", user_id);
        println!("StateChain ID: {:?}", statechain_id);
        ...
        println!("kp.party_2_public: {:?}", kp.party_2_public);
        println!("o2_pub: {:?}", o2_pub);

        println!("p1_pub: {:?}", p1_pub);
        println!("p1_pub: {:?}", p2_pub);
        ...
        let scsig = StateChainSig {
            purpose: "TRANSFER".to_string(), // "TRANSFER", "TRANSFER-BATCH" or "WITHDRAW"
            data: "DATA".to_string(),    // proof key, state chain id or address
            sig: "sig".to_string(),
        };

        let finalized_data = TransferFinalizeData {
            new_shared_key_id: user_id.clone(),
            statechain_id: statechain_id.clone(),
            statechain_sig: scsig,
            s2,
            new_tx_backup_hex: "transfer_msg4.tx_backup_hex".to_string(),
            batch_data: None,
        };
    ...
    fn keyupdate_second(&self, ku_fin: KUFinalize) -> Result<KUAttest>{
        ...
        println!("New user ID: {:?}", ku_fin.shared_key_id);
        println!("StateChain ID: {:?}", ku_fin.statechain_id);

        self.database.new_user_finalize(&ku_fin.shared_key_id, &ku_fin.statechain_id)?;

        let ku_attest = KUAttest {
            statechain_id: ku_fin.statechain_id,
...
#[post("/ecdsa/keyupdate/first", format = "json", data = "<ku_send>")]
pub fn keyupdate_first(
    sc_entity: State<SCE>,
    ku_send: Json<KUSendMsg>,
) -> Result<Json<KUReceiveMsg>> {
    match sc_entity.keyupdate_first(ku_send.into_inner()) {
        Ok(res) => return Ok(Json(res)),
        Err(e) => return Err(e),
    }
}

#[post("/ecdsa/keyupdate/second", format = "json", data = "<ku_fin>")]
pub fn keyupdate_second(
    sc_entity: State<SCE>,
    ku_fin: Json<KUFinalize>,
) -> Result<Json<KUAttest>> {
    match sc_entity.keyupdate_second(ku_fin.into_inner()) {
        Ok(res) => return Ok(Json(res)),
        Err(e) => return Err(e),
    }
}
